...
def showvideo(request):
    ...
    if request.method == 'POST' and 'upload' in request.POST:
        ...
        colored_srt_filepath = os.path.join(settings.MEDIA_ROOT, 'captions', 'colored_audio.srt')
        ...
        with open(colored_srt_filepath, 'w') as f:
            f.writelines(lines)

        # Put subtitles on the video and output the new captioned video
        ff = FFmpeg(
            inputs={input_file_path: None},
            outputs={output_file_path: f'-vf subtitles={colored_srt_filepath} -y'}
        )
        ff.run()
